...
public class LambdaService {
    ...
    public InstructorLeadClassData getInstructorLeadClassData(String classId) {
        ...
        if (records.size() > 0) {
            return new InstructorLeadClassData(records.get(0).getClassId(), records.get(0).getName(),
                    records.get(0).getDescription(), records.get(0).getClassType(), records.get(0).getUserId(),
                    records.get(0).getClassCapacity(), records.get(0).getDateTime().toString(),
                    records.get(0).isStatus());
        }
        return null;
    }
    ...
    public InstructorLeadClassData setInstructorLeadClassData(String name, String description, String classType,
            String userId, int classCapacity, String dateTime, boolean status) {
        String id = UUID.randomUUID().toString();
        InstructorLeadClassRecord record = exampleDao.setInstructorLeadClassData(id, name, description, classType,
                userId, classCapacity, dateTime, status);
        return new InstructorLeadClassData(id, name, description, classType, userId, classCapacity, dateTime.toString(),
                status);
    }

    public InstructorLeadClassData updateInstructorLeadClassData(String classId, String name, String description,
            String classType, String userId, int classCapacity, String dateTime, boolean status) {
        InstructorLeadClassRecord record = exampleDao.updateInstructorLeadClassData(classId, name, description,
                classType, userId, classCapacity, dateTime, status);
        return new InstructorLeadClassData(classId, name, description, classType, userId, classCapacity,
                dateTime.toString(), status);
    }
    ...
    public UserData getUserData(String username) {
        ...
        if (records.size() > 0) {
            return new UserData(records.get(0).getUserId(), records.get(0).getFirstName(), records.get(0).getLastName(),
                    records.get(0).getUserType(), records.get(0).getMembership(), records.get(0).getStatus(),
                    records.get(0).getUsername(), records.get(0).getPassword());
        }
        return null;
    }
    ...
    public UserData setUserData(String data) {
        ...
        return new UserData(record.getUserId(), record.getFirstName(), record.getLastName(), record.getUserType(),
                record.getMembership(), record.getStatus(), record.getUsername(), record.getPassword());
    }
    ...
    public ClassAttendanceData getClassAttendanceData(String userId, String classId) {
        ...
        if (records.size() > 0) {
            return new ClassAttendanceData(records.get(0).getUserId(), records.get(0).getClassId(),
                    records.get(0).getAttendanceStatus());
        }
        return null;
    }
